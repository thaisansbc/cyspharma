$(document).ready(function(e) {
    ...
    $('#srref').change(function(e) {
        localStorage.setItem('srref', $(this).val());
    });
    if ((srref = localStorage.getItem('srref'))) {
        $('#srref').val(srref);
    }
    if ($('#slwarehouse').val()) {
...
function loadItems() {
    if (localStorage.getItem('slitems')) {
        ...
        $.each(sortedItems, function() {
            ...
            if (item.row.code == 'sub_qty') {
                ...
                tr_html =
                    ...
                    ' - ' + item_name + ((item_expiry   != null && site.settings.product_expiry == 1) ? ' (' + item_expiry + ')' : '') + (sel_opt != '' ? ' (' + sel_opt + ')' : '') + '</span>'
                        + '<i class="pull-right fa fa-edit tip pointer edit" id="' +row_no +'" data-item="' +item_id +'" title="Edit" style="cursor:pointer;' + ((item.free) ? "display:none;" : "") + '"></i>'; 
                ...
                if (site.settings.product_serial == 1) {
                    tr_html +=
                        '<td class="text-right"><input class="form-control input-sm rserial" name="max_serial[]" type="text" id="max_serial_' +
                        row_no +
                        '" value="' +
                        item_max_serial +
                        '"></td>';
                ...
                if (site.settings.show_qoh == 1) {
                    tr_html +=
                        '<td class="text-right"><input class="form-control text-center" type="text" id="qoh_' +
                        row_no +
                        '" value="' +
        ...
        var tfoot =
            '<tr id="tfoot" class="tfoot active"><th colspan="' + col + '">Total</th><th class="text-center">' +
