...
function suppliers(ele, su) {
    ...
    $(ele).select2({
        ...
        $('#supplier_'+su+'_part_no').val(e.object.code || '');
    });
}
function customers(ele, su) {
       console.log(su);
    $(ele).select2({
        minimumInputLength: 1,
        ajax: {
        ...
        $('#customer_'+su+'_part_no').val(e.object.code || '');
    });
}
...
function row_status(x) {
    if (x == null) {
        return '';
    } else if (x == 'issue' || x == 'shortlist' || x == 'order' || x == 'permission' || x == 'pending' || x == 'deposited' || x == 'cleared' || x == 'booked' || x == 'requested' || x == 'applied' || x == 'reservation' || x == 'repairing') {
        return '<div class="text-center"><span class="row_status label label-warning">' + lang[x] + '</span></div>';
    ...
    } else if (x == 'void' || x == 'candidate' || x == 'voiced' || x == 'absent' || x == 'checked'  || x == 'unpublic'|| x == 'spoiled' || x == 'difference' || x == 'due' || x == 'returned' || x == 'rejected' || x == 'inactive' || x == 'payoff' || x == 'pawn_sent' || x == 'closed' || x == 'no' || x == 'expired' || x == 'deleted' || x == 'take_away' || x == 'checked_out' || x == 'declined' || x == 'suspended' || x == 'cancelled' || x == 'not_done') {
        return '<div class="text-center"><span class="row_status label label-danger">' + lang[x] + '</span></div>';
    } else if (x == 'black_list') {
        return '<div class="text-center"><span class="row_status label label-other" style="background-color:#2e2e2d">' + lang[x] + '</span></div>';
    } else {
...
$(document).ready(function () {
    ...
    $('body').on('click', '.pickslip_link td:not(:first-child, :nth-child(10), :nth-last-child(2), :last-child)', function() {
        $('#myModal').modal({
            remote: site.base_url +'sales/modal_view_pickslip/' +$(this).parent('.pickslip_link').attr('id'),
        });
        $('#myModal').modal('show');
    });
    ...
    $('body').on('click', '.credit_note_link td:not(:first-child, :nth-child(10), :nth-last-child(2), :last-child)', function() {
        $('#myModal').modal({remote: site.base_url + 'account/view_credit_note/' + $(this).parent('.credit_note_link').attr('id')});
        $('#myModal').modal('show');
    });
