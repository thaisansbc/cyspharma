...
function loadItems() {
    if (localStorage.getItem('qaitems')) {
        ...
        $('#add_adjustment, #edit_adjustment').attr('disabled', false);
        console.log(sortedItems);
        $.each(sortedItems, function () {
            var item = this;
            var item_id = site.settings.item_addition == 1 ? item.item_id : item.id;
            item.order = item.order ? item.order : new Date().getTime();
            var product_id = item.row.id, item_qty = item.row.qty, item_option = item.row.option, item_type = item.row.product_type,
            ...
            tr_html = '<td><input name="product_id[]" type="hidden" class="rid" value="' + product_id + '"><span class="sname" id="name_' + row_no + '">' + item_code +' - ' + item_name + ' (' + product_unit_name + ')' + '</span></td>';
            ...
            if (item_type == 'standard') {
                var new_arr = {};
                if (qaitems != null) {
                    $.each (qaitems, function(index, obj) {
                        if (obj.row.id == product_id && obj.expiry == item_expiry && obj.row.type == 'subtraction') {
                ...
                if (item.pitems) {
                    $.each(item.pitems, function () {
                        if (Object.keys(new_arr) == (this.product_id + (this.expiry != null ? ('_' + this.expiry) : ''))) {
                            if (parseInt(Object.values(new_arr)) > parseInt(this.quantity_balance) && type == "subtraction") {
                                $('#row_' + row_no).addClass('danger');
                                if (site.settings.overselling != 1 || (site.settings.overselling == 1 && whOverselling != 1)) {
                                    $('#add_adjustment, #edit_adjustment').attr('disabled', true);
                                }
                            }                        
                        }                        
                    });
        ...
        if (site.settings.show_qoh == 1) { tfoot += '<th></th>'; }
