        ...
        if (site.settings.select_price == 1) {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $.each(item.units, function() {
                        if (this.id == unit) {
                            $('#pcost').val(formatDecimal((parseFloat(item.row.base_unit_cost)))).change();
                        }
                    });
                }
            });
            ...
            if(unit != poitems[item_id].row.base_unit) {
                $.each(item.units, function() {
                    if (this.id == unit) {
                        $('#pcost').val(formatDecimal((parseFloat(item.row.base_unit_cost) * (unitToBaseQty(1, this))))).change();
                    }
                });
            } else {
...
function loadItems() {
    if (localStorage.getItem('poitems')) {
        ...
        $.each(sortedItems, function () {
            ...
            var qoh = item.row.qoh ? item.row.qoh : 0;
                ...
                if(item.row.fup != 1 && product_unit != item.row.base_unit) {
                    $.each(item.units, function(){
                        if (this.id == product_unit){
                            ...
                            if(site.settings.select_price == 1){ 
                                if (product_unit == item.row.purchase_unit) {
                                    unit_cost = formatDecimal((parseFloat(item.row.base_unit_cost)));
                                } else {
                                    ...
                                    $.each(item.set_price, function () {
                                        if (this.unit_id == l.id) {
                                            if (item.row.base_unit_cost == this.cost){
                                                unit_cost = formatDecimal((parseFloat(this.cost)));
                                            }else{ 
                                                unit_cost = formatDecimal((parseFloat(item.row.cost)));
                                            }
                                        }
                                    });
                                }
            ...
            if (site.settings.tax1 == 1) {
                tr_html += '<td class="text-right show-cost"><input class="form-control input-sm text-right rproduct_tax" name="product_tax[]" type="hidden" id="product_tax_' + row_no + '" value="' + pr_tax.id + '"><span class="text-right sproduct_tax" id="sproduct_tax_' + row_no + '">' + (pr_tax_rate ? '(' + pr_tax_rate + ')' : '') + ' ' + formatMoney(pr_tax_val * item_qty) + '</span></td>';
            }
            tr_html += '<td class="text-right show-cost"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + formatMoney(((parseFloat(item_cost) + parseFloat(pr_tax_val)) * parseFloat(item_qty))) + '</span></td>';
        ...
        // }
        // if (site.settings.product_discount == 1) {
        //     tfoot += '<th class="text-right show-cost">'+formatMoney(product_discount)+'</th>';
        // }
        if (site.settings.tax1 == 1) {
            tfoot += '<th class="text-right show-cost">'+formatMoney(product_tax)+'</th>';
        }
        tfoot += '<th class="text-right show-cost">'+formatMoney(total)+'</th>';
        if ((!Admin && !Owner) && show_cost == 0) { tfoot += '<th></th>'; }
