...
$(document).on("change", '.rack_id', function () {
    ...
    var selected_rack_id = $(this).val(); // Get the selected rack ID
    var item_id = $row.attr('data-item-id'); // Fetch the data-item-id attribute

    // Update poitems with the selected rack_id
    poitems[item_id].row.rack_id = selected_rack_id;

    // Store updated data back to localStorage
    localStorage.setItem('poitems', JSON.stringify(poitems));

    // Optionally reload the items to reflect changes
    loadItems();
});
...
function loadItems() {
    if (localStorage.getItem('poitems')) {
        ...
        $.each(sortedItems, function () {
            ...
            // Retrieve the rack ID already selected for this row
            var selectedRackId = poitems[item_id]?.row?.rack_id || ''; 
            
            rack.forEach(function(itemrack) {
                var isSelected = itemrack.id == selectedRackId ? 'selected' : ''; // Check if this rack is selected
        ...
        var col = (site.settings.using_weight == 1 ? 2:1) + add_col;
