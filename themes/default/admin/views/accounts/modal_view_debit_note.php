                    ...
                    <tbody>
                        ...
                        ?>
                            <tr>
                                ...
                                <td style="width: 80px; text-align:center; vertical-align:middle;"><?= abs($this->bpas->formatQuantity($row->unit_quantity)); ?></td>
                                <td style="width: 80px; text-align:center; vertical-align:middle;"><?= $row->product_unit_code; ?></td>
                                <?php
                                if ($inv->status == 'partial') {
                                    echo '<td style="text-align:center;vertical-align:middle;width:80px;">' . abs($this->bpas->formatQuantity($quantity_received)). '</td>';
                                } ?>
                                <?php if ($Settings->default_currency == "USD") { ?>
                                ...
                                <td style="text-align:right; width:120px;"><?= $this->bpas->formatMoney(abs($row->subtotal)); ?></td>
                            </tr>
                            <?php
                            $r++;
                        endforeach;
                    ...
                    <tfoot>
                        ...
                        <?php if ($inv->grand_total != $inv->total) { ?>
                            <tr>
                                ...
                                <td style="text-align: right;"><?= $this->bpas->formatMoney($return_purchase ? abs(($inv->total + $inv->product_tax) + ($return_purchase->total + $return_purchase->product_tax)) : abs($inv->total + $inv->product_tax)); ?></td>
                            </tr>
                        <?php } ?>
                        <?php
                        if ($return_purchase) {
                        ...
                        <?php if ($inv->order_discount != 0) {
                            echo '<tr><td colspan="' . $col . '"  style="text-align: right;">' . lang('order_discount') . ' (' . $default_currency->code . ')</td><td  style="text-align: right;">' . ($inv->order_discount_id ? '<small>(' . $inv->order_discount_id . ')</small> ' : '') . $this->bpas->formatMoney($return_purchase ? abs($inv->order_discount + $return_purchase->order_discount) : abs($inv->order_discount)) . '</td></tr>';
                        }
                        ?>
                        <?php if ($Settings->tax2 && $inv->order_tax != 0) {
                            echo '<tr><td colspan="' . $col . '" style="text-align: right;">' . lang('order_tax') . ' (' . $default_currency->code . ')</td>
                                    <td style="text-align: right;">' . $this->bpas->formatMoney($return_purchase ? abs($inv->order_tax + $return_purchase->order_tax) : abs($inv->order_tax)) . '</td></tr>';
                        }
                        ?>
                        ...
                        <tr>
                            ...
                            <td style="text-align:right; font-weight:bold;"><?= $this->bpas->formatMoney($return_purchase ? abs($inv->grand_total + $return_purchase->grand_total) : abs($inv->grand_total)); ?></td>
                        </tr>
                        <tr>
                            <td colspan="<?= $col; ?>" style="text-align:right; font-weight:bold;"><?= lang('paid'); ?>
                                (<?= $default_currency->code; ?>)
                            </td>
                            <td style="text-align:right; font-weight:bold;"><?= $this->bpas->formatMoney($return_purchase ? abs($inv->paid + $return_purchase->paid) : abs($inv->paid)); ?></td>
                        </tr>
                        <tr>
                            ...
                            <td style="text-align:right; font-weight:bold;">
                                ...
                                <?php 
                                $total_balance = ($return_purchase ? ($inv->grand_total + $return_purchase->grand_total) : $inv->grand_total) - ($return_purchase ? ($inv->paid + $return_purchase->paid) : $inv->paid);
                                
                                echo $this->bpas->formatMoney(abs($total_balance)); ?></td>
                        </tr>
