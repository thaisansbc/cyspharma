...
<script>
    $(document).ready(function () {
        oTable = $('#POData').dataTable({
            ...
            'sAjaxSource': '<?= admin_url('purchases_order/getPurchasesOrder'.($biller_id ? '/'.$biller_id : '').'?v=1'.$v); ?>',
            ...
            'fnRowCallback': function (nRow, aData, iDisplayIndex) {
				...
				}else if(aData[9]=="requested" || aData[9]=="pending"){
					action.find('.unapproved').remove();
					action.find('.unreject').remove();
                    action.find('.create').remove();
				}else if(aData[9]=="reject"){
					action.find('.edit').remove();
					action.find('.create').remove();
            ...
            {column_number: 6, filter_default_label: "[<?=lang('eta');?>]", filter_type: "text", data: []},
...
<?php if ($Owner || $GP['bulk_actions']) {
	    echo admin_form_open('purchases_order/purchase_actions', 'id="action-form"');
	}
?>
<div class="breadcrumb-header">
    <?php $biller_title = ($biller_id ? $biller->name : ((isset($user_biller) && !empty($user_biller)) ? $user_biller->name : lang('all_billers'))); ?>
    <h2 class="blue"><i class="fa-fw fa fa-star"></i><?=lang('purchase_order') . ' (' . $biller_title . ')';?></h2>
        ...
        <ul class="btn-tasks">
            <li class="dropdown">
                ...
                <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                    ...
                    <li class="divider"></li>
                    <li>
                        <a href="#" class="bpo" title="<?=lang("delete_purchases_order")?>"
            ...
            <?php if (($this->Owner || $this->Admin) || empty($count_billers)) { ?>
                <li class="dropdown">
                    ...
                    <ul class="dropdown-menu pull-right" class="tasks-menus" role="menu" aria-labelledby="dLabel">
                        <li><a href="<?= admin_url('purchases_order') ?>"><i class="fa fa-building-o"></i> <?= lang('all_billers') ?></a></li>
                        <li class="divider"></li>
                        <?php
                        foreach ($billers as $biller) {
                            echo '<li><a href="' . admin_url('purchases_order/' . $biller->id) . '"><i class="fa fa-building"></i>' . $biller->company.'/'.$biller->name . '</a></li>';
                        } ?>
                    </ul>
                </li>
    ...
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
				...
				<div id="form">
					<?php  echo admin_form_open("purchases_order"); ?>
                    <div class="row">
                        <div class="col-sm-4">
                            <div class="form-group">
                                <label class="control-label" for="reference_no"><?= lang("po_no"); ?></label>
                    ...
                    <table id="POData" cellpadding="0" cellspacing="0" border="0" class="table table-hover table-striped">
                        <thead>
                            <tr class="active">
                                ...
                                <th class="show-cost"><?= lang("grand_total"); ?></th>
                                <th><?= lang("status"); ?></th>
