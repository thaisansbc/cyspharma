...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        <?php if (isset($quote_id) && !empty($quote_id)) { ?>
            //localStorage.setItem('qudate', '<?= $this->bpas->hrld($quote->date) ?>');
            localStorage.setItem('qucustomer',   '<?= $quote->customer_id ?>');
            localStorage.setItem('qubiller',     '<?= $quote->biller_id ?>');
            localStorage.setItem('qusaleman_by', '<?= isset($quote->saleman_by) ? $quote->saleman_by : '' ?>');
            localStorage.setItem('quwarehouse',  '<?= $quote->warehouse_id ?>');
            localStorage.setItem('qunote',       '<?= str_replace(["\r", "\n"], '', $this->bpas->decode_html($quote->note)); ?>');
            localStorage.setItem('qudiscount',   '<?= $quote->order_discount_id ?>');
            localStorage.setItem('qutax2',       '<?= $quote->order_tax_id ?>');
            localStorage.setItem('qushipping',   '<?= $quote->shipping ?>');
            localStorage.setItem('quitems',      JSON.stringify(<?= $quote_items; ?>));
                                ...
                                <div class="panel-body" style="padding: 5px;">
                                    ...
                                    <div class="col-md-4 hide">
                                        <div class="form-group">
                                            ...
                                            $valid_day = date('d/m/Y', strtotime("+28 days"));
                                            ?>
                                            <?= lang('valid_day', 'valid_day'); ?>
                                            <?php echo form_input('valid_day', $valid_day, 'class="form-control input-tip date" id="valid_day"'); ?>
                                        </div>
                                    </div>
                                    <div class="col-md-3 hide">
                                        <div class="form-group">
                                            <?= lang('service', 'service'); ?>
                                            <?php echo form_input('service', (isset($_POST['service']) ? $_POST['service'] : ''), 'class="form-control input-tip" id="quref"'); ?>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-sm-6">
                                <div class="col-sm-12">
                                    <div class="form-group">
                                        ...
                                        <?php echo form_textarea('payment_note', (isset($_POST['payment_note']) ? $_POST['payment_note'] :''), 'class="form-control" id="payment_note" style="margin-top: 10px; height: 100px;"'); ?>
                                    </div>
                                </div>
                            </div>
                            <div class="col-sm-6">
