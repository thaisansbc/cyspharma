...
<script>
    $(document).ready(function() {
        oTable = $('#SLData').dataTable({
            ...
            'bProcessing': true,
            'bServerSide': true,
            'sAjaxSource': '<?= admin_url('sales/getPickSlip' . ($biller_id ? '/' . $biller_id : '') . '?v=1' . $v . ($this->input->get('shop') ? '&shop=' . $this->input->get('shop') : '') . ($this->input->get('attachment') ? '&attachment=' . $this->input->get('attachment') : '') . ($this->input->get('delivery') ? '&delivery=' . $this->input->get('delivery') : '')); ?>',
            ...
            'fnRowCallback': function(nRow, aData, iDisplayIndex) {
                ...
                nRow.className = "pickslip_link re" + aData[11];
                //if(aData[7] > aData[9]){ nRow.className = "product_link warning"; } else { nRow.className = "product_link"; }

                var action = $('td:eq(15)', nRow);              
                if (aData[8] == 'completed' && aData[12] == 'paid') {
...
<div class="breadcrumb-header">
    ...
    <h2 class="blue"><i class="fa-fw fa fa-heart"></i><?= lang('pick_slip') . ' (' . $biller_title . ')';?></h2>
    </h2>
    <div class="box-icon">
        <ul class="btn-tasks">
            <li class="dropdown">
        ...
        <ul class="btn-tasks">
            <li class="dropdown">
                ...
                <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                    <li>
                        <a href="<?= admin_url('sales/add_pickslip') ?>">
                            <i class="fa fa-plus-circle"></i> <?= lang('add_pickslip') ?>
                        </a>
                    </li>
                    <li>
