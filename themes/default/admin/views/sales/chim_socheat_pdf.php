                ...
                <div class="col-xs-5">
                    <table style="font-size: 14px;line-height:25px;">
						...
						<tr>
							<td>កាលបរិច្ឆេទ </td>
							<td>&nbsp;&nbsp;:&nbsp;&nbsp;</td>
							<td><b><?= $this->bpas->hrld($invs->date); ?></b></td>
						</tr>
					</table>
                </div>
            </div>
				...
				<div class="col-sm-12 col-xs-12">
					<table class="table" style="margin-top: 10px;">
						<tbody style="font-size: 14px;">
                                   ...
                                   $getvar = $this->sales_model->getAllProductVarain($row->product_id);
										 ...
										 foreach($getvar as $varian){
											 if($varian->product_id){
												...
												$var = $this->bpas->sales_model->getVariantName($row->product_id,$row->option_id);
												$str_unit = $var->name;
							...
							?>
								<tr>
									...
									<?php if ($row->item_discount) {?>
										<td style="vertical-align: middle; text-align: center">
										$<?=$this->bpas->formatMoney($row->item_discount);?></td>
									<?php } ?>
									<td style="vertical-align: middle; text-align: right"> <?= $row->subtotal!=0 ? $this->bpas->formatMoney($row->subtotal):$free; 
									$total += $row->subtotal;
									?>
									</td>
								</tr>
							...
							<tr>
								<td colspan="3" rowspan="1" style="border-left: 1px solid #FFF !important;  ">អត្រាប្រាក់​ 1$ =<b>&nbsp;&nbsp;<?=$this->bpas->formatMoney($rates->rate);?></b></td>
							...
							<tr>
								...
								<td style="text-align:right; vertical-align:middle;">
										$<?=$this->bpas->formatMoney($invs->order_tax);?>
								</td>
							</tr>
							<?php }?>
							...
							<tr>
								...
								<td align="right">$<?php echo $this->bpas->formatMoney($invs->deposit); ?></td>
							</tr>
							<?php } ?>
							<?php //$this->bpas->print_arrays($invs->paid);?>
							<tr>
								<td colspan="<?= $col; ?>" style="text-align: right; font-weight: bold;height:30px;">ប្រាក់ដែលត្រូវបង់ 
								</td>
								<td align="right">$<?= $this->bpas->formatMoney($grand_total=$invs->total-$invs->order_discount+$invs->order_tax); ?></td>
							</tr>
							<?php if($invs->paid >0) { ?>
							<tr>
								<td colspan="<?= $col; ?>" style="text-align: right; font-weight: bold;height:30px;">បានបង់ 
								</td>
								<td align="right">$<?= $this->bpas->formatMoney($invs->paid); ?></td>
							</tr>
							<?php } ?>
							...
							<tr>
								...
								<td align="right">$<?= $this->bpas->formatMoney($invs->grand_total-$invs->paid); ?></td>
							</tr>
							<?php } ?>
							...
							<tr>
								...
								<td align="right">$<?= $this->bpas->formatMoney($invs->paid-$invs->total); ?></td>
							</tr>
							<?php } ?>
						</tbody>
