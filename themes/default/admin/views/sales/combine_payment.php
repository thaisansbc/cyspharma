...
<div class="modal-dialog">
    <div class="modal-content">
        ...
        echo admin_form_open_multipart("sales/combine_payment_receivable/", $attrib); ?>
			...
			<div class="table-responsive">
                <table id="CompTable" cellpadding="0" cellspacing="0" border="0"
                    ...
                    <tr>
                        ...
                        <th style="width:18%; display:none;"><?= $this->lang->line("discount"); ?></th>
                        <th style="width:20%;"><?= $this->lang->line("amount_paid"); ?></th>
						<td style="text-align: center !important; cursor: default;"><i class="fa fa-trash-o"></i></td>
                    </tr>
                    </thead>
                    <tbody>
                    <?php if (!empty($combine_sales)) {
                        ...
                        foreach ($combine_sales as $combine_sale) { ?>
                            <tr class="row<?= $combine_sale->id ?>">
                                <td><?= $this->bpas->hrld($combine_sale->date); ?></td>
                                <td><?= $combine_sale->reference_no; ?></td>                           
                                <td class="balance" val='<?=$combine_sale->balance?>'><?= $this->bpas->formatDecimal($combine_sale->balance); ?></td>
                                <td style="display:none;"><input type="text" name="discount_paid[]" class="discount_paid form-control" value="0" /></td>
                                <td class="col-xs-3">
                                    <input type="hidden" name="sale_id[]" class="form-control" value="<?= $combine_sale->id; ?>" id="amount_">
                                    <input type="text" name="amount_paid_line[]" class="amount_paid_line form-control" value="<?= $this->bpas->formatDecimal($combine_sale->balance); ?>">
                                </td>
                    ...
                    <tfoot>
                        <tr>
							...
							<td style="display:none;"><strong id="total_discount"><?= $this->bpas->formatDecimal(0); ?></strong></td>
                            <td><strong id="paid_amount"><?= $this->bpas->formatDecimal($tamountpaid); ?></strong></td>
                                ...
                                <div class="payment">
                                    <div class="form-group">
                                        ...
                                        <input name="amount-paid" type="text" id="amount_1"
                                            value="<?= $this->bpas->formatDecimal($total); ?>" class="pa form-control kb-pad amount" required="required"/>
                                    </div>
                                </div>
                            </div>
                            <div class="col-sm-6">
            ...
            if (cn != '') {
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        } else if (data.customer_id !== null && data.customer_id != "<?= (isset($inv->customer_id) && $inv->customer_id != '') ? $inv->customer_id : ''?>") {
                            $('#gift_card_no').parent('.form-group').addClass('has-error');
                            bootbox.alert('<?=lang('gift_card_not_for_customer')?>');

                        } else {
                            var due = <?= isset($inv) ? ($inv->grand_total - $inv->paid) : 0 ?>;
                            if (due > data.balance) {
