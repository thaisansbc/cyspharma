...
<script>
    $(document).ready(function () {
        oTable = $('#id_cardData').dataTable({
            ...
            'sAjaxSource': '<?= admin_url('hr/getIDCards/'.($biller ? $biller->id : '')); ?>',
			...
			'fnRowCallback': function (nRow, aData, iDisplayIndex) {
				...
				var action = $('td:eq(7)', nRow);
				if(aData[5] == 'pending'){
					action.find('.unapprove_id_card').remove();
					action.find('.print_id_card').remove();
				}else if(aData[5] == 'approved'){
					action.find('.approve_id_card').remove();
					action.find('.edit_id_card').remove();
					action.find('.delete_id_card').remove();
				}
            },
            "aoColumns": [{"mRender": checkbox},{"mRender": fld}, {"mRender": fld}, null, null,{"mRender": row_status},{"bSortable": false,"mRender": attachment},  {"bSortable": false}]
        }).fnSetFilteringDelay().dtFilter([
			{column_number: 1, filter_default_label: "[<?=lang('date');?> (yyyy-mm-dd)]", filter_type: "text", data: []},
            {column_number: 2, filter_default_label: "[<?=lang('invalid_date');?>]", filter_type: "text", data: []},
...
<div class="box">
    <div class="box-header">
		<h2 class="blue"><i class="fa-fw fa fa-heart-o"></i><?= lang('id_cards').' ('.($biller ? $biller->name : lang('all_billers')).')' ?></h2>
        <div class="box-icon">
            <ul class="btn-tasks">
                <li class="dropdown">
                    ...
                    <ul class="dropdown-menu pull-right" class="tasks-menus" role="menu" aria-labelledby="dLabel">
                        <li>
                            <a href="<?= admin_url('hr/add_id_card') ?>"><i class="fa fa-plus-circle"></i> <?= lang('add_id_card') ?>
                            </a>
                        </li>
                        <li>
