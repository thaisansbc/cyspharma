            ...
            <ul class="btn-tasks">
				...
				<li class="dropdown hide">
                    <a data-toggle="dropdown" class="dropdown-toggle" href="#"><i
                            class="icon fa fa-industry tip" data-placement="left"
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div id="form">
                    ...
                    <?php echo admin_form_open("reports/cash_books"); ?>
                    <div class="row">
                        <div class="col-sm-3">
                            <div class="form-group">
                                <label class="control-label" for="user"><?= lang("billers"); ?></label>
                                <?php
                                $bill["0"] = lang("all");
                                foreach ($billers as $biller) {
                                    $bill[$biller->id] = $biller->company." " .$biller->name;
                                }
                ...
                <div class="table-scroll">
                    <table id="registerTable" cellpadding="0" cellspacing="0" border="0" class="table table-hover table-striped reports-table table-condensed">
						<thead>
							<tr>
								...
								foreach($acc as $val){
									...
									if($gltran_list) { ?>
										<tr>
											...
											<td colspan="3"><b>
												<?=$this->bpas->formatMoney($startAmount->startAmount)?>
											</b></td>
										</tr>
										<?php		
										...
										foreach($gltran_list as $rw){
											...
											<tr>
												...
												<td><div class="fix-text text-center"><?=($rw->amount > 0 ? $this->bpas->formatMoney($rw->amount) : '0.00')?></div></td>
												<td><div class="fix-text text-center"><?=($rw->amount < 1 ? $this->bpas->formatMoney(abs($rw->amount)) : '0.00')?></div></td>
											</tr>
											<?php
										} ?>
										<tr>
											...
											<td colspan="3"><b><?=$this->bpas->formatMoney($endAccountBalance)?></b></td>
										</tr>
										<?php
									}
								}
...
<script type="text/javascript">
    $(document).ready(function () {
		...
		*/
		$('#pdf').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=admin_url('reports/cash_books/pdf')?>";
            return false;
        });
        $('#xls').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=admin_url('reports/cash_books/0/0/xls/?v=1'.$v)?>";
            return false;
        });
