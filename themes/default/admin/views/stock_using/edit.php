<script type="text/javascript">
    ...
    $(document).ready(function () {
		...
		if (localStorage.getItem('biller')) {
	...
	<?php if ($items) {?>
		...
		localStorage.setItem('biller', '<?= $using_stock->biller_id; ?>');
		localStorage.setItem('account', '<?= $using_stock->account; ?>');
		localStorage.setItem('plan', '<?= $using_stock->plan_id; ?>');
	<?php } ?>
</script>
<?php echo admin_form_open("products/edit_using_stock/".$using_stock->id."/use"); ?>
<div class="box">
    <div class="box-header">
        <h2 class="blue">
			<i class="fa-fw fa fa-heart"></i><?= lang('edit_stock_using'); ?> 
		</h2>
    </div>
				...
				<div class="row">
					<div class="col-md-4">
						<div class="form-group">
							<?= lang('date', 'date'); ?>
							<?= form_input('date', $this->bpas->hrld($date), 'class="form-control tip datetime" required id="date" autocomplete=off'); ?>
						</div>
					</div>
					<div class="col-md-4">
						<div class="form-group">
