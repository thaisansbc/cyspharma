...
<script>
    $(document).ready(function () {
        oTable = $('#dmpData').dataTable({
            ...
            {column_number: 5, filter_default_label: "[<?=lang('approved_date');?>]", filter_type: "text", data: []},
            {column_number: 6, filter_default_label: "[<?=lang('approved_by');?>]", filter_type: "text", data: []},
        ...
        <?php if ($this->session->userdata('remove_qals')) { ?>
            ...
            <?php $this->bpas->unset_data('remove_qals');
        }
        ?>
    });
</script>
...
<div class="breadcrumb-header">
    ...
    <h2 class="blue"><i class="fa-fw fa fa-filter"></i><?= lang('quantity_adjustments') . ' (' . $wh_title . ')'; ?></h2>
    <div class="box-icon">
        <ul class="btn-tasks">
            <li class="dropdown">
                ...
                <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                    ...
                    <li>
                        <a href="<?= admin_url('products/add_adjustment_by_excel') ?>">
                            <i class="fa fa-plus-circle"></i> <?= lang('add_adjustment_by_excel') ?>
                        </a>
                    </li>
                    <li>
                    ...
                    <li class="divider"></li>
                    <li>
                        <a href="#" class="bpo" title="<b><?= $this->lang->line('delete_adjustments') ?></b>"
                            data-content="<p><?= lang('r_u_sure') ?></p><button type='button' class='btn btn-danger' id='delete' data-action='delete'><?= lang('i_m_sure') ?></a> <button class='btn bpo-close'><?= lang('no') ?></button>"
                            data-html="true" data-placement="left">
                        <i class="fa fa-trash-o"></i> <?= lang('delete_adjustments') ?>
                         </a>
                     </li>
                </ul>
            </li>
            ...
            <?php } elseif (!empty($warehouses)){ ?>
                <li class="dropdown">
                    ...
                    <ul class="dropdown-menu pull-right" class="tasks-menus" role="menu" aria-labelledby="dLabel">
                        ...
                        <li class="divider"></li>
                        <?php
                        $warehouse_id_ = explode(',', $this->session->userdata('warehouse_id'));
                        foreach ($warehouses as $warehouse) {
                            foreach ($warehouse_id_ as $key => $value) {
                    ...
                    <table id="dmpData" class="table table-bordered table-condensed table-hover table-striped">
                        <thead>
                            <tr>
                                ...
                                <th class="col-xs-2"><?= lang('adjustment_date'); ?></th>
                                <th class="col-xs-2"><?= lang('adjustment_no'); ?></th>
                                <th class="col-xs-2"><?= lang('warehouse'); ?></th>
                                <th class="col-xs-2"><?= lang('note'); ?></th>
                                <th class="col-xs-2"><?= lang('approved_date'); ?></th>
                                <th class="col-xs-2"><?= lang('approved_by'); ?></th>
