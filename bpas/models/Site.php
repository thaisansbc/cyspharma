...
public function getCompanyByBillers($group_name)
        ...
        if ($q->num_rows() > 0) {
            ...
            switch ($field) {
                ...
                case 'gen':
                    ...
                    break;
                case 'pick':
                    $prefix = $this->Settings->pickslip_prefix;
                    break;
                default:
                    $prefix = '';
            }
    ...
    // public function getProductUniteByID($id)
    // {
    //     if (!empty($id)) {
    //         if ($product = $this->getProductByID($id)) {
    //             return $this->getProdutUnitsByBUID($product->unit, $product->id);
    //         }
    //     }
    //     return false;
    // }
    
    public function getProdutUnitsByBUID($unite_id, $product_id)
    {
        $q = $this->db->get_where('product_units', ['unit_id' => $unite_id, 'product_id' => $product_id], 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return false;
  ...
  public function getschedulesByID($id) 
    ...
    public function getDistrictsArea($city_id = false){
        $this->db->where("IFNULL(district_id,0)",0);
    ...
    public function getCommunesArea($district_id = false){
        $this->db->where("IFNULL(commune_id,0)",0);
        $this->db->where('city_id >',0);
