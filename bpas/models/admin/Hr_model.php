...
class Hr_model extends CI_Model
{
	...
	public function addPromotion($data = array())
	{
		if($this->db->insert('hr_employees_working_promote',$data)){
			...
			$this->RecordEmployeeHistory('promotion','add',$data['employee_id'],$Pdata);
	...
	public function RecordEmployeeHistory($type=null,$status=null,$employee_id=false,$data=[]){
        ...
        //     $this->db->update('hr_employees_history', array('quantity' => $wh_balance_qty), array('product_id' => $product_id, 'warehouse_id' => $warehouse_id));

        //     return TRUE;
        // } else {
        //     if( ! $wh_balance_qty) { $wh_balance_qty = 0; }
        //     $product = $this->getProductByID($product_id);
        //     $this->db->insert('hr_employees_history', array('quantity' => $wh_balance_qty, 'product_id' => $product_id, 'warehouse_id' => $warehouse_id, 'avg_cost' => $product->cost));
