...
//        $this->default_biller_id = $this->site->default_biller_id();
    ...
    public function syncPR($po_id = false, $pr_id = false){
        ...
        if($pr_id){
            ...
            $this->db->update("purchases_request",array("status"=>$status),array("id"=>$pr_id));
        }
    }
    ...
    public function getRefPurchaseRequests($status = false){
        ...
        if($status){
            if($status=="approved"){
                $this->db->where_in('purchases_request.status',array("approved","partial"));
            }else{
                $this->db->where('purchases_request.status',$status);
            }
        }
        if (!$this->Owner && !$this->Admin && $this->session->userdata('warehouse_id')) {
            $this->db->where_in('purchases_request.warehouse_id',json_decode($this->session->userdata('warehouse_id')));
        }
        if (!$this->Owner && !$this->Admin && $this->session->userdata('biller_id')) {
            $this->db->where('purchases_request.biller_id',$this->session->userdata('biller_id'));
        }
