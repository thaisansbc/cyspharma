...
class Sales_model extends CI_Model
{
    ...
    public function getItemRack($product_id, $warehouse_id)
    {
        ...
        $q = $this->db->get_where('warehouses_products', ['product_id' => $product_id, 'warehouses_products.warehouse_id' => $warehouse_id], 1);
        if ($q->num_rows() > 0) {
            $wh = $q->row();
            return $wh->rack;
        }
    ...
    public function getProductNames($term, $warehouse_id, $pos = false, $limit = 15,$none_stock=false)
    {
        ...
        if ($this->Settings->multiple_code_unit != 0) {
            ...
            $this->db->select("
                ...
                ->group_by('products.id');
            if ($none_stock || ($this->Settings->overselling && (!empty($warehouse) && $warehouse->overselling))) {
                $this->db->where("
                    (
    ...
    {
		$this->db->select('sales.project_id,sales.customer_id,sales.biller_id,sales.id,sales.date,sales.reference_no,sales.grand_total, IFNULL(bms_payments.paid,0) as paid, IFNULL(bms_payments.discount,0) as discount,total_return,paid_return,sales.ar_account')
    ...
    public function getPickSlipByID($id)
    {
        $q = $this->db->get_where('pickslip', ['id' => $id], 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return false;
    }
     public function getPickSlipItems($sale_id, $return_id = null)
    {
        $this->db->select('pickslip_items.*, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate, 
                product_variants.name as variant, 
                units.code as base_unit_code,
                units.name as unit_name,
                products.slug,
                ...
                sale_units.name as product_unit_name
            ')
        ->join('products', 'products.id=pickslip_items.product_id', 'left')
        ->join('product_variants', 'product_variants.id=pickslip_items.option_id', 'left')
        ->join('tax_rates', 'tax_rates.id=pickslip_items.tax_rate_id', 'left')
        ->join('units', 'units.id=products.unit', 'left')
        ->join('units sale_units', 'sale_units.id=pickslip_items.product_unit_id', 'left')
        ->join('options', 'options.id=pickslip_items.option_comment_id', 'left')
        ->group_by('pickslip_items.id')
        ->order_by('id', 'asc');
        if ($sale_id && !$return_id) {
            $this->db->where('pick_id', $sale_id);
        } elseif ($return_id) {
            $this->db->where('pick_id', $return_id);
        }
        $q = $this->db->get('pickslip_items');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function addPickSlip($data = [], $items = [], $stockmoves = []){  
        $this->db->trans_start();
        if ($this->db->insert('pickslip', $data)) {
            $sale_id = $this->db->insert_id();
         
            if ($this->site->getReference('pick') == $data['reference_no']) {
                $this->site->updateReference('pick');
            }
        
            foreach ($items as $item) {
                $item['pick_id'] = $sale_id;
                $this->db->insert('pickslip_items', $item);
                $sale_item_id = $this->db->insert_id();
                if ($item['quantity'] < 0) {
                    if ($this->Settings->accounting_method == '2') {
                        $cal_cost = $this->site->updateAVGCost($item['product_id']);
            ...
            //$this->site->sendTelegram("pickslip",$sale_id,"added");
        }
        $this->db->trans_complete();
        if ($this->db->trans_status() === false) {
            log_message('error', 'An errors has been occurred while adding the sale (Add:Sales_model.php)');
    ...
    public function UpdatePickSlip($id, $data, $items = [], $stockmoves = []){  
        $this->db->trans_start();

        if ($this->db->update('pickslip', $data, ['id' => $id]) && 
            $this->db->delete('pickslip_items', ['pick_id' => $id]) && $this->db->delete('sale_combo_items', ['pick_id' => $id])) {
    ...
    public function delete_pickslip($id)
    {
        $this->db->trans_start();
        if ($this->db->delete('pickslip_items', ['pick_id' => $id]) && 
            $this->db->delete('pickslip', ['id' => $id])) {
            $this->db->trans_complete();
            if ($this->db->trans_status() === false) {
                log_message('error', 'An errors has been occurred while adding the sale (Delete:Sales_model.php)');
            } else {
