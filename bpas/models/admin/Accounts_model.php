...
class Accounts_model extends CI_Model
{
    ...
    public function addCreditNote($data = [], $items = [], $stockmoves = [], $payment = [], $si_return = [], $accTrans = array(), $accTranPayments = array(), $payment_status = null, $commission_product = null, $module = null){  
        ...
        if ($this->db->insert('credit_note', $data)) {
            ...
            if ($data['payment_status'] == 'partial' || $data['payment_status'] == 'paid' && !empty($payment)) {
                $payment['credit_note_id']      = $credit_note_id;
                ...
                if ($accTranPayments) {
                    foreach ($accTranPayments as $accTranPayment) {
                        $accTranPayment['tran_no']      = $credit_note_id;
                ...
                $this->site->syncSalePayments($credit_note_id);
            }
            if ($stockmoves) {
                foreach ($stockmoves as $stockmove) {
                    if ($stockmove['product_type'] != 'combo') {
                        $stockmove['transaction_id'] = $credit_note_id;
    ...
    {
        $this->db->trans_start();
        if ($this->db->delete('credit_note_items', ['credit_note_id' => $id]) && 
            $this->db->delete('credit_note', ['id' => $id])) {
            $this->db->trans_complete();
            if ($this->db->trans_status() === false) {
    ...
    public function getCreditNotePayments($sale_id)
    {
        $this->db->where('transaction is NULL');
        $q = $this->db->get_where('payments', ['credit_note_id' => $sale_id]);
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function syncCreditNotePayments($id = null){
        if ($id) {
            $sale = $this->getCreditNoteByID($id);
            if ($payments = $this->getCreditNotePayments($id)) {
                $paid        = 0;
                ...
                if ($this->db->update('credit_note', ['paid' => 0, 'payment_status' => $payment_status], ['id' => $id])) {
                    return true;
                }
            }
            return false;
    ...
    public function deleteCreditNotePayment($id){
        ...
        if ($this->db->delete('payments', ['id' => $id])) {
            ...
            if ($opay->paid_by == 'gift_card') {
                ...
                $this->db->update(
                    'companies', ['deposit_amount' => ($customer->deposit_amount + $opay->amount)], 
                    ['id' => $customer->id]);
            }
            return true;
        }
    ...
    public function addCreditNotePayment($data = [], $customer_id = null, $accTranPayments = array())
    {
        if ($this->db->insert('payments', $data)) {
            $payment_id = $this->db->insert_id();
            $sales = $this->getCreditNoteByID($data['credit_note_id']);
    ...
    public function EditCreditNotPayment($id, $data = [], $customer_id = null, $accTranPayments = array())
    {
        $opay = $this->site->getPaymentByID($id);
    ...
    public function getCreditNoteItems($credit_note_id, $return_id = null)
    {
        ...
        ->order_by('id', 'asc');
        if ($credit_note_id && !$return_id) {
            $this->db->where('credit_note_id', $credit_note_id);
        } elseif ($return_id) {
            $this->db->where('credit_note_id', $return_id);
        }
    ...
    {
        $this->db->select('debit_note_items.*, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate, 
   ...
   public function addDebitNote($data, $items, $accTrans = null, $return_id = null, $stockmoveReturns = false)
    {
        $this->db->trans_start();
        if ($this->db->insert('debit_note', $data)) {
            ...
            foreach ($items as $item) {
                $item['debit_note_id'] = $purchase_id;
    ...
    public function updateDebitNote($id, $data, $items = [], $accTrans = null)
    {
        ...
        if ($this->db->update('debit_note', $data, ['id' => $id]) && $this->db->delete('debit_note_items', ['debit_note_id' => $id])) {
            $purchase_id = $id;
            $this->site->deleteStockmoves('debit_note', $purchase_id);
            $this->site->deleteAccTran('debit_note', $purchase_id);
            if ($accTrans) {
                $this->db->insert_batch('gl_trans', $accTrans);
            }
    ...
    public function addDebitPayment($data = [], $accTranPayments = array())
    {   
        if ($this->db->insert('payments', $data)) {
            //---------accounting-----
            $payment_id = $this->db->insert_id();
    ...
    public function getRefPurchases($status = false){
