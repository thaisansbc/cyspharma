...
class Products_api extends CI_Model
{
    ...
    public function getProductCountItemByCode($id, $product_code = null)
        {
            return $this->db->get_where('stock_count_items', ['stock_count_id' => $id, 'product_code'=> $product_code])->result();
        }
    ...
    public function getStockCountById($count_id = null)
    {
        if ($count_id !== null) {
            $this->db->where('stock_count_id', $count_id); 
        }
    
        return $this->db->get('stock_count_items')->result(); 
    }

    public function getStockCount($id = null)
    {
        return $this->db->get_where('stock_counts', ['id' => $id], 1)->row();
    }
    ...
    public function updateStockCount($count,$items)
    {
        if($count){
            $id=$count['id'];
            unset($count['id']);
            $countstock=$this->getStockCount($id);
            if($countstock){
                $this->db->update('stock_counts', $count, ['id' => $id]); 
                $date= date('Y-m-d H:i:s');
                if($items){
                    for ($i=0; $i < count($items); $i++) { 
                        $item_expiry = ((isset($items[$i]['expiry']) && $items[$i]['expiry'] != '0000-00-00' && $items[$i]['expiry'] != 'null' && $items[$i]['expiry'] != 'NULL' && $items[$i]['expiry'] != false) ? $items[$i]['expiry'] : null);
                        $this->db->set('date_time', $date);
                        $this->db->set('counted', $items[$i]['countStock']);
                        $this->db->where('stock_count_id', $id);
                        $this->db->where('product_code', $items[$i]['product_code']);
                        $this->db->where('product_id', $items[$i]['product_id']);
