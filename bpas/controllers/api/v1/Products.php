...
class Products extends REST_Controller
{
    ...
    {
        if ($this->input->server('REQUEST_METHOD') == 'GET') {
            $code = $this->get('product_code');
            $id = $this->get('id');
     
            if ($getsetting = $this->products_api->getProductCountItemByCode($id, $code)) {
                $this->response($getsetting, REST_Controller::HTTP_OK);
            } else {
                $this->response([
                    'message' => 'No expense record found.',
    ...
    public function updateCount_post()
    {
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            $jdata = json_decode(file_get_contents('php://input'),true);
            ...
            if(json_last_error() === JSON_ERROR_NONE) {
                ...
                $items      = $this->post('countStockitems');
                $countStock = $this->post('countStock');
                $id         = $countStock['id'];
                
                $checkCountStock=$this->products_api->getStockCount($id);
                if ($checkCountStock->status == 1) {
                    $this->response("stock_count_finalized.", REST_Controller::HTTP_BAD_REQUEST);
                }else{
                    $StockCount = $this->products_api->updateStockCount($countStock,$items);

                    if($StockCount==true){
                        $this->response([
                            'status' => TRUE,
                            'message' => 'stock_count_finalized.',
                            'data' => $StockCount
                        ], REST_Controller::HTTP_OK);
                    }else{
                        $this->response("Some problems occurred, please try again.", REST_Controller::HTTP_BAD_REQUEST);
