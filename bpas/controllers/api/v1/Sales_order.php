...
class Sales_order extends REST_Controller
{
    ...
    public function approved_status_post(){
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            $jdata = json_decode(file_get_contents('php://input'),true);//json_decode($this->input->raw_input_stream, true);
            if(json_last_error() === JSON_ERROR_NONE) {

                $sale_order_id  = $jdata['sale_order_id'];
                $date           = $jdata['date'];
                $approve_status = $jdata['sale_order_status'] ? $jdata['sale_order_status']:'approved';

                $data = [
                   // 'approved_date'     => $date,
                    'order_status'      => $approve_status,
                //    'approved_by'       => $this->session->userdata('user_id'),
                 //   'sale_order_id'     => $sale_order_id,
                ];
                $insert = $this->sales_order_api->UpdateSaleOrderStatus($sale_order_id,$data);
                if($insert){
                    $this->response([
                        'status' => TRUE,
                        'message' => 'Sale Order has been approved successfully.',
                        'data' => $insert
                    ], REST_Controller::HTTP_OK);
                }else{
                    $this->response("Some problems occurred, please try again.", REST_Controller::HTTP_BAD_REQUEST);
