...
class system_settings extends MY_Controller
{
    ...
    public function add_category()
    {
        ...
        if (SHOP) {
            ...
            $this->form_validation->set_rules('slug', lang('slug'));
        }
    ...
    public function zones(){
        $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
        $this->data['getZones'] = $this->settings_model->getAllZones();
    ...
    {
        $zone_group        = $this->input->get('zone_group') ? $this->input->get('zone_group') : null;
        ...
        $this->datatables
            ->select("{$this->db->dbprefix('zones')}.id as id, 
                ...
                GROUP_CONCAT({$this->db->dbprefix('areas')}.name) as commune
                ", false)
            ->from('zones')
            ->join('zones z', 'z.id=zones.parent_id', 'left')
            ->join('custom_field', 'custom_field.id=zones.zone_group_id', 'left')

            ->join("(SELECT id,name FROM ".$this->db->dbprefix('areas')." WHERE IFNULL(city_id,0) = 0) as cities","cities.id = zones.city_id","left")

            ->join("(SELECT id,name FROM ".$this->db->dbprefix('areas')." WHERE city_id > 0 AND IFNULL(district_id,0) = 0) as districts","districts.id = zones.district_id","left")

        //    ->join("areas dis","find_in_set(dis.id,{$this->db->dbprefix('zones')}.district_id)<> 0","left")
            //->where('dis.cit > ',0)
            

            ->join("areas","find_in_set({$this->db->dbprefix('areas')}.id,{$this->db->dbprefix('zones')}.commune_id)<> 0","left")
           // ->group_by('areas.id')
            ...
            if ($zone_group) {
                $this->datatables->where('zones.zone_group_id', $zone_group);
            }
        echo $this->datatables->generate();
    }
    ...
    public function add_zone()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $cities     = implode(",", $this->input->post('city_id[]'));
            $districts  = implode(",", $this->input->post('district_id[]'));
            $commune    = implode(",", $this->input->post('commune_id[]'));
            $data = [
        ...
        if ($this->form_validation->run() == true && $this->settings_model->addZone($data)) {
           ...
           redirect($_SERVER['HTTP_REFERER']);
            ...
            $this->data['cities']       = $this->site->getCitiesArea();
            $this->data['districts']    = $this->site->getDistrictsArea();
            $this->data['communes']     = $this->site->getCommunesArea();
    ...
    function send_telegram($id = NULL)
    {
        ...
        if($send["ok"]){
            ...
            redirect("system_settings/telegrams");
        }else{
   ...
   function promotion()
    ...
    public function getRacks()
    {
        ...
        $this->datatables->select("
                ...
                c.name as parent", false)
            ->from('product_rack')
            ->join('warehouses c', 'c.id=product_rack.warehouse_id', 'left')
    ...
    {
        $this->load->library('datatables');
        $delete_area = "<a href='#' class='tip po' title='<b>" . lang("delete_area") . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete111111' href='" . admin_url('system_settings/delete_area/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a>";
        $this->datatables
            ...
            ->add_column("Actions", "<div class=\"text-center\"><a href='" . admin_url('system_settings/edit_area/$1') . "' class='tip' title='" . lang("edit_area") . "' data-toggle='modal' data-backdrop='static' data-keyboard='false' data-target='#myModal'><i class=\"fa fa-edit\"></i></a> {$delete_area}</div>", "id");
        echo $this->datatables->generate();
    }
    ...
    function add_area()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect("system_settings/areas");
        }
    ...
    function edit_area($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true && $this->settings_model->updateArea($id, $data)) {
            ...
            $this->data['area'] = $area;
            $this->data['cities'] = $this->site->getCitiesArea();
            $this->data['districts'] = $area->city_id > 0 ? $this->site->getDistrictsArea($area->city_id) : false;
            $this->data['communes'] = $area->district_id > 0 ? $this->site->getCommunesArea($area->district_id) : false;
            $this->data['modal_js'] = $this->site->modal_js();
    ...
    function delete_area($id = NULL)
    {
     ...
     public function get_district_area(){
        $city_id = $this->input->get('city_id');
        $districts = $this->site->getDistrictsArea($city_id);
        echo json_encode($districts);
    }
    public function get_commune_area(){
        $district_id = $this->input->get('district_id');
        $communes = $this->site->getCommunesArea($district_id);
        echo json_encode($communes);
    }
    function payment_condition()
    {
        $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');

        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('system_settings'), 'page' => lang('system_settings')), array('link' => '#', 'page' => lang('payment_condition')));
        $meta = array('page_title' => lang('payment_condition'), 'bc' => $bc);
        $this->page_construct('settings/payment_condition', $meta, $this->data);
    }
    
    function getPaymentCondition(){
        $this->load->library('datatables');
        $this->datatables
            ->select("id, description, noted")
            ->from("payment_term")
            ->where('type','condition')
            ->order_by('id', 'asc')
            ->add_column("Actions", "<center><a href='" . admin_url('system_settings/edit_payment_condition/$1') . "' class='tip' title='" . lang("edit_payment_condition") . "' data-toggle='modal' data-target='#myModal'><i class=\"fa fa-edit\"></i></a> <a href='#' class='tip po' title='<b>" . lang("delete_payment_condition") . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('system_settings/delete_payment_condition/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></center>", "id");
        //->unset_column('id');

        echo $this->datatables->generate();
    }
    public function add_payment_condition()
    {
        $this->form_validation->set_rules('description', lang("description"), 'trim|required');
        if ($this->form_validation->run() == true) {
            $data = array(
                ...
                'type'          => 'condition',
                'noted'         => $this->bpas->clear_tags($this->input->post('description'))
            );
        } elseif ($this->input->post('add_payment_term')) {
            $this->session->set_flashdata('error', validation_errors());
            redirect($_SERVER["HTTP_REFERER"]);
        ...
        if ($this->form_validation->run() == true && $this->settings_model->addPaymentTerm($data)) {
            ...
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['modal_js'] = $this->site->modal_js();
            $this->data['page_title'] = lang("payment_condition");
            $this->load->view($this->theme . 'settings/add_payment_condition', $this->data);
        }
    }
    public function edit_payment_condition($id)
    {
        $this->form_validation->set_rules('description', lang("description"), 'trim|required');
        if ($this->form_validation->run() == true) {
             $data = array(
        ...
        if ($this->form_validation->run() == true && $this->settings_model->updatePaymentTerm($id, $data)) {
            ...
            $this->data['id'] = $id;
            $this->data['modal_js'] = $this->site->modal_js();
            $this->data['page_title'] = lang("edit_payment_condition");
            $this->data['data'] = $this->settings_model->getPaymentTermById($id);
            $this->load->view($this->theme . 'settings/edit_payment_condition', $this->data);
        }
    }
    public function delete_payment_condition($id)
    {
        //$this->session->set_flashdata('message', lang("payment_term_deleted");
        if($this->db->delete('payment_term', array('id' => $id))){
            $this->bpas->send_json(['error' => 0, 'msg' => lang('payment_condition_deleted')]);
            redirect($_SERVER["HTTP_REFERER"]);
        }
    }
}