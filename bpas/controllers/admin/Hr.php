...
class Hr extends MY_Controller
{
	...
	function getEmployees($biller_id = null)
    {
        ...
        $this->datatables
            ->select("
				...
				concat(lastname_kh,' ',firstname_kh) as name_kh,
				hr_employees.gender,
	...
	public function dimension_size($id){
		...
		if ($this->form_validation->run() == true) {
			...
			$data = array(
				...
				'profile_padding_bottom' => $this->input->post('profile_padding_bottom') ? $this->input->post('profile_padding_bottom') : 0,
	...
	public function getIDCards($biller_id = false){
		...
		$this->datatables->select("	hr_id_cards.id as id, 
					...
					")
			->from("hr_id_cards")
			->join('hr_positions',"hr_positions.id = hr_id_cards.position_id","left")
			->join("hr_departments","hr_departments.id = hr_id_cards.department_id","left")
   ...
   public function travels_report(){
	...
	public function add_on_boarding($employee_id = false)
	{
		...
		{	
			...
			if ($_FILES['experience_etter_attachment']['size'] > 0) {
                $this->load->library('upload');
                $config['upload_path'] = $this->digital_upload_path;
                $config['allowed_types'] = $this->digital_file_types;
                $config['max_size'] = $this->allowed_file_size;
                ...
                $config['encrypt_name'] = TRUE;
                $this->upload->initialize($config);
                if (!$this->upload->do_upload('experience_etter')) {
                    $error = $this->upload->display_errors();
                    $this->session->set_flashdata('error', $error);
                    admin_redirect($_SERVER["HTTP_REFERER"]);
                }
                $photo = $this->upload->file_name;
                $data['experience_etter_attach'] = $photo;
            }
	...
	public function getoffBoarding($employee_id = false)
	{	
		if(!$employee_id){
			$employee_id = $this->input->get("employee_id");
		}
