...
class purchases_request extends MY_Controller
{
    ...
    public function getPurchasesRequested($biller_id = null)
    {
        ...
        $this->datatables
            ->select("
                purchases_request.id as id,
                DATE_FORMAT({$this->db->dbprefix('purchases_request')}.date, '%Y-%m-%d %T') as date,
                companies.company, 
                reference_no, note,order_status , required_date,purchases_request.status")
