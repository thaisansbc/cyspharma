...
class Purchases extends MY_Controller
{
    ...
    public function getPurchases($biller_id = null)
    {
        ...
        $action .=  '<li>' . $payments_link . '</li> 
            ...
            <li>' . $email_link . '</li>
            <li>' . $print_barcode . '</li>
            <li class="hide">' . $add_debit_note . '</li>
            <li>' . $return_link . '</li>
            <li>' . $delete_link . '</li>
        </ul>
        </div></div>';
        ...
        $this->datatables->where('purchases.debit_note',0);
        if ($biller_id) {
    ...
    public function suggestions()
    {
        ...
        $ware_id = $this->input->get('warehouse', true);
        ...
        if ($rows) {
            ...
            foreach ($rows as $row) {
                ...
                $categories = $this->site->getCategoryByID($cate_id);
                $set_price  = $this->site->getUnitByProId($row->id);
                $rack       = $this->purchases_model->getracksbywarehouse($ware_id);
                $pr[] = [
                    'id'  => sha1($c . $r), 'item_id' => $row->id, 'label' => $row->name . ' (' . $row->code . ')',
                    'row' => $row, 'tax_rate' => $tax_rate, 'set_price' => $set_price, 'units' => $units, 'options' => $options, 'fiber' => $categories, 'rack' => $rack,
                ];
                $r++;
            }
        ...
        if ($this->form_validation->run() == true) {
            ...
            foreach($sale_id_arr as $sale_id){
                ...
                if($payment['amount'] > 0 ){
                    ...
                    if($this->Settings->module_account == 1){
                        ...
                        //$this->db->insert('gl_trans',$accTranPayments);
                        //$this->db->insert('gl_trans',$accTranPayments_debit);
                    }
                    //=====end accountig=====//
            ...
            if ((!$this->Owner && !$this->Admin) && 
                ...
                if($supplier_balance == "supplier_balance"){
                    admin_redirect('purchases/supplier_balance');
                }elseif($payable == "payable"){
                    admin_redirect('account/list_ac_payable');
                }else{
                    admin_redirect('purchases');
                }
            }
            ...
            if ($id) {
                ...
                foreach ($inv_items as $item) {             
                    ...
                    if ($row->received != 0) {
                        ...
                        $tax_rate              = $this->site->getTaxRateByID($row->tax_rate);
                        $rack       = $this->purchases_model->getracksbywarehouse($purchase->warehouse_id);
                        $ri                    = $this->Settings->item_addition ? $row->id : $c;
                        $pr[$ri] = array(
                            'id'  => $c, 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 
                            'row' => $row, 'tax_rate' => $tax_rate, 'units' => $units, 'options' => $options,'rack'=>$rack);
                        $c++;
                    }
                }
                $this->data['inv'] = $purchase;
