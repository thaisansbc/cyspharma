...
class Account extends MY_Controller{
    ...
    public function payment_note($id = null) 
    {
        ...
        $inv = $this->accounts_model->getCreditNoteByID($payment->credit_note_id);
   ...
   //     $this->bpas->checkPermissions('payments', true);
    ...
    public function getCreditNote($biller_id = false)
    {
        ...
        $delete_link = "<a href='#' class='po' title='<b>" . lang("void_credit_note") . "</b>' data-content=\"<p>"
        ...
        $this->datatables->select("{$this->db->dbprefix('credit_note')}.id as id,
            ...
            (ABS(IFNULL(".$this->db->dbprefix('credit_note').".grand_total,0)) - ABS(IFNULL(bpas_payments.paid,0))- ABS(IFNULL(bpas_payments.discount,0)) + ABS(IFNULL(invoice_interest,0)) - ABS(IFNULL(surcharge_interest,0))) as balance,

            {$this->db->dbprefix('credit_note')}.payment_status,
            attachment,
            return_id")
            ->from('credit_note')
            ...
            ->join('(SELECT
                    ...
                    GROUP BY
                        credit_note_id) as bpas_payment_inv', 'bpas_payment_inv.credit_note_id=credit_note.id', 'left')
            ->join('(SELECT
                        credit_note_id,
                    ...
                    FROM
                        '.$this->db->dbprefix('payments').'
                    GROUP BY
                        credit_note_id) as bpas_payments', 'bpas_payments.credit_note_id=credit_note.id', 'left');
    ...
    {   
        ...
        if ($this->form_validation->run() == true) {
            if ($this->Owner || $this->Admin || $this->GP['change_invoiceNo']) {
                $reference = $this->site->CheckedSaleReference($this->input->post('reference_no'), $this->site->getReference('re'));
            } else {
        ...
        if ($this->form_validation->run() == true && $this->accounts_model->addCreditNote($data, $products, $stockmoves, $payment, $si_return = array(), $accTrans, $accTranPayments, null, $commission_product)) {
            ...
            $this->data['slnumber']        = $this->site->getReference('re');
    ...
    public function edit_credit_note($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->accounts_model->EditCreditNote($id, $data, $products, $stockmoves, $accTrans, $accTranPayments, $commission_product)) {
            $this->session->set_userdata('remove_slls', 1);
           ...
           // $this->bpas->deadlineDayEditing($this->data['inv']->date);
            ...
            foreach ($inv_items as $item) {
                ...
                $row->name            = $item->product_name;
                $row->type            = $item->product_type;
                $row->base_quantity   = $item->quantity *(-1);
                $row->expiry          = $item->expiry;
    ...
    public function void_credit_note($id = null)
    {
        $this->bpas->checkPermissions(null, true);
        if ($this->input->get('id')) {
            $id = $this->input->get('id');
        }
        if ($this->accounts_model->void_credit_note($id)) {
            $this->session->set_flashdata('message', lang('credit_note_has_been_voided'));
            admin_redirect('account/credit_note');
        }
    }
    public function view_credit_note($id = null)
    {
        ...
        $this->data['warehouse'] = $this->site->getWarehouseByID($inv->warehouse_id);
        $this->data['inv'] = $inv;
        $this->data['payment'] = $this->accounts_model->getCreditNotePayments($inv->id);
        $this->data['sale_payments'] = $this->accounts_model->getCreditNotePayments($inv->sale_id);
        $this->data['rows']         = $this->accounts_model->getCreditNoteItems($id);
        if($this->Owner || $this->Admin || $this->bpas->GP['unlimited-print']){
            $this->data['print'] = 0;
        }else{
        ...
        $this->load->view($this->theme . 'sales/modal_view_credit_note', $this->data);
    }
    ...
    public function getDebitNote($biller_id = null)
    {
        ...
        $user_query     = $this->input->get('user')?$this->input->get('user'):NULL;
        $reference_no   = $this->input->get('reference_no')?$this->input->get('reference_no'):NULL;
        $purchase_no    = $this->input->get('purchase_no')?$this->input->get('purchase_no'):NULL;
        $supplier       = $this->input->get('supplier')?$this->input->get('supplier'):NULL;
        $warehouse      = $this->input->get('warehouse')?$this->input->get('warehouse'):NULL;
        ...
        $a                  = $this->input->get('a') ? $this->input->get('a') : null;
        $detail_link        = anchor('admin/account/modal_view_debit_note/$1', '<i class="fa fa-money"></i> ' . lang('debit_note_details'), 'data-toggle="modal" data-backdrop="static" data-target="#myModal"');
        ...
        $delete_link        = "<a href='#' class='po' title='<b>" . $this->lang->line('void_debit_note') . "</b>' data-content=\"<p>"
        ...
        <ul class="dropdown-menu pull-right" role="menu">
            ...
            <li>' . $add_payment_link . '</li>
            <li>' . $edit_link . '</li>
            <li class="hide">' . $pdf_link . '</li>
            <li class="hide">' . $email_link . '</li>
            <li>' . $delete_link . '</li>
        </ul>
        </div></div>';
        ...
        $this->datatables
            ->select("debit_note.id, 
                ...
                abs({$this->db->dbprefix('debit_note')}.paid) as paid, 
                abs({$this->db->dbprefix('debit_note')}.grand_total-{$this->db->dbprefix('debit_note')}.paid) as balance, 
        ...
        $this->datatables->join('debit_note pu', 'pu.id = debit_note.purchase_id', 'left');
        ...
        if ($user_query) {
            $this->datatables->where('debit_note.created_by', $user_query);
        }
        if ($supplier) {
            $this->datatables->where('debit_note.supplier_id', $supplier);
        }
        if ($warehouse) {
            $this->datatables->where('debit_note.warehouse_id', $warehouse);
        }
        if ($reference_no) {
            $this->datatables->like('debit_note.reference_no', $reference_no, 'both');
        }
        if ($purchase_no) {
            $this->datatables->like('debit_note.pu', $purchase_no, 'both');
        }
        if ($start_date) {
            $this->datatables->where($this->db->dbprefix('debit_note').'.date BETWEEN "' . $start_date . '" and "' . $end_date . '"');
        }
        if ($project) {
            $this->datatables->like('debit_note.project_id', $project);
        }
        if ($status) {
            $this->datatables->like('debit_note.status', $status);
        }
        if ($note) {
            $this->datatables->like('debit_note.note', $note, 'both');
        }
     ...
     public function add_debit_note($purchase_id = null)
    {
        ...
        $order_referent = $this->purchases_model->getPurchaseByID($purchase_id);
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = [
             ...
             //   'purchase_id'       =>  !empty($this->input->post('pu_reference'))? $this->input->post('pu_reference') :'',
                'biller_id'         => $biller_id,
                'project_id'        => $this->input->post('project'),
                'reference_no'      => $reference,
    ...
    public function edit_debit_note($id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $item_discount      = isset($_POST['product_discount'][$r]) ? $_POST['product_discount'][$r] : null;
                $supplier_part_no   = (isset($_POST['part_no'][$r]) && !empty($_POST['part_no'][$r])) ? $_POST['part_no'][$r] : null;
                $quantity_balance   = (0-$_POST['quantity_balance'][$r]);
    ...
    public function modal_view_debit_note($purchase_id = null)
    {
        ...
        $this->data['created_by']      = $this->site->getUser($inv->created_by);
        $this->data['updated_by']      = $inv->updated_by ? $this->site->getUser($inv->updated_by) : null;
        $this->data['return_purchase'] = $inv->return_id ? $this->accounts_model->getDebitNoteByID($inv->return_id) : null;
        $this->data['return_rows']     = $inv->return_id ? $this->accounts_model->getAllDebitNoteItems($inv->return_id) : null;
        $this->data['biller']      = $this->site->getCompanyByID($inv->biller_id);
    ...
    public function debit_payments($id = null,$ex_id = null)
    {
        ...
        $this->load->view($this->theme . 'accounts/debit_payments', $this->data); 
    }
    public function add_debit_payment($id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            if ($this->purchases_model->addDebitPayment($payment,$accTranPayments)) {
                $this->session->set_flashdata('message', lang('payment_added'));
                admin_redirect('account/debit_note');
            }else{
