...
class Attendances extends MY_Controller
{
	...
	public function add_take_leave()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
				...
				$leave_type = isset($_POST['leave_type'][$r])? $_POST['leave_type'][$r]: '';
				if($leave_type){
					$dataDetails[] = array(
        ...
        if ($this->form_validation->run() == true && $this->attendances_model->addTakeLeave($data, $dataDetails)) {
            ...
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('attendances'), 'page' => lang('attendance')), array('link' => admin_url('attendances/take_leaves'), 'page' => lang('take_leaves')), array('link' => '#', 'page' => lang('add_take_leave')));
	...
	public function edit_take_leave($id)
    {
		...
		if($take_leave->status !=0){
			$this->session->set_flashdata('error', lang('take_leave_cannot_edit'));
            $this->bpas->md();
		}
        $this->form_validation->set_rules('biller', lang("biller"), 'required');
        ...
        if ($this->form_validation->run() == true && $this->attendances_model->updateTakeLeave($id, $data, $dataDetails)) {
            $this->session->set_userdata('remove_tls', 1);
            ...
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('attendances'), 'page' => lang('attendance')),array('link' => admin_url('attendances/take_leaves'), 'page' => lang('take_leaves')), array('link' => '#', 'page' => lang('edit_take_leave')));
