...
class Products extends MY_Controller
{
    ...
    function getProducts($warehouse_id = NULL)
    {
        ...
        $pcf5 = $this->input->get('pcf5') ? $this->input->get('pcf5') : null;
        $pcf6 = $this->input->get('pcf6') ? $this->input->get('pcf6') : null;
        ...
        if ($pcf5) {
            $this->datatables->like($this->db->dbprefix('products') . ".pcf5", $pcf5);
        }
        if ($pcf6) {
            $this->datatables->like($this->db->dbprefix('products') . ".pcf6", $pcf6);
        }
    ...
    public function add($id = null, $type = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $product_units[] = array(
                ...
                'unit_qty'     => $this->input->post('unit_number'),
                'unit_price'   => $this->input->post($punit->code."_price"),
            ); 
            ...
            $data  = [
                ...
                'supplier_product_code1'               => $this->input->post('supplier_product_code'),
                'supplier_product_code2'               => $this->input->post('supplier_product_code2'),
                'supplier_product_code3'               => $this->input->post('supplier_product_code3'),
                'supplier_product_code4'               => $this->input->post('supplier_product_code4'),
                'supplier_product_code5'               => $this->input->post('supplier_product_code5'),
                'customer1'         => $this->input->post('customer'),
                'customer2'         => $this->input->post('customer_2'),
                'customer3'         => $this->input->post('customer_3'),
                'customer4'         => $this->input->post('customer_4'),
                'customer5'         => $this->input->post('customer_5'),
                'customer1_part_no' => $this->input->post('customer_part_no'),
                'customer2_part_no' => $this->input->post('customer_2_part_no'),
                'customer3_part_no' => $this->input->post('customer_3_part_no'),
                'customer4_part_no' => $this->input->post('customer_4_part_no'),
                'customer5_part_no' => $this->input->post('customer_5_part_no'),
                'customer_product_code1'               => $this->input->post('customer_product_code'),
                'customer_product_code2'               => $this->input->post('customer_product_code2'),
                'customer_product_code3'               => $this->input->post('customer_product_code3'),
                'customer_product_code4'               => $this->input->post('customer_product_code4'),
                'customer_product_code5'               => $this->input->post('customer_product_code5'),
                ...
                'moh_license_expiry_days'          => $this->input->post('moh_license_expiry_days') ? $this->bpas->fsd($this->input->post('moh_license_expiry_days')) : null,
    ...
    public function edit($id = null, $type=null)
    {
        ...
        foreach($units as $unit) {
            ...
            $product_units[] = array(
                'product_id'    => $id,
                'unit_id'       => $unit->id,
                'unit_qty'      => $this->input->post('unit_number') ? $this->input->post('unit_number') : $unitProduct->unit_qty,
                'unit_price'    => $this->input->post($unit->code . '_price'),
            );
        }
        if ($this->input->post('type') == 'standard') {
            $this->form_validation->set_rules('unit', lang('product_unit'), 'required');
            $this->form_validation->set_rules($default_sale_unit->code."_cost", lang('product_cost'));
        ...
        if ($this->form_validation->run('products/add') == true) {
            ...
            $data  = [
                ...
                'width'             => $this->input->post('width'),
                'height'            => $this->input->post('height'),
                'lenghth'           => $this->input->post('lenghth'),
                'weight'            => $this->input->post('weight'),                         
                'width'            => $this->input->post('width'),                         
                'height'            => $this->input->post('height'),         
                'max_serial'        => $this->input->post('max_serial'),
        ...
        if ($this->form_validation->run() == true && $this->products_model->updateProduct($id, $data, $items, $warehouse_qty, $product_attributes, $photos, $update_variants, $addOn_items, $product_options, $unit_arr, $product_account, $bom_items, $formulation_items, $product_units, $warehouse_racks)) {
            ...
            $this->data['unitProduct']             = $unitProduct;
    ...
    public function getadjustments($warehouse_id = null)
    {
        ...
        $this->datatables
            ->select("
                {$this->db->dbprefix('adjustments')}.id as id, date, 
                reference_no, 
    ...
    public function get_using_stock()
    {
        ...
        $this->datatables
            ->select("bpas_enter_using_stock.id as id,bpas_enter_using_stock.date, bpas_enter_using_stock.reference_no as refno,
            bpas_companies.company, bpas_projects_plan.title as home_type, CONCAT(bpas_products.cf3, ', ',bpas_products.cf4) as address, bpas_warehouses.name as warehouse_name, bpas_users.username, bpas_enter_using_stock.note, bpas_enter_using_stock.type as type", FALSE)
            ->from("enter_using_stock")
            ->join($qi, 'FQI.using_stock_id=enter_using_stock.id', 'left')
            ->join('companies', 'bpas_companies.id=bpas_enter_using_stock.biller_id', 'inner')
            ->join('warehouses', 'bpas_enter_using_stock.warehouse_id=bpas_warehouses.id', 'left')
            ->join('projects_plan', 'bpas_enter_using_stock.plan_id = bpas_projects_plan.id', 'left')
            ->join('products', 'bpas_enter_using_stock.address_id = bpas_products.id', 'left')
            ->join('users', 'bpas_users.id=bpas_enter_using_stock.employee_id', 'inner')
            ->order_by('bpas_enter_using_stock.date', 'desc')
            ->order_by('bpas_enter_using_stock.reference_no', 'desc');
            if($fdate && $tdate){
    ...
    public function add_using_stock($purchase_id = null, $id = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
			...
			// 	$checkExpiry = $this->site->checkExpiry($stockmoves, null, 'UsingStock');
