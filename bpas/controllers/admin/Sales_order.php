...
class Sales_order extends MY_Controller
{
        ...
        if ($this->form_validation->run() == true && $this->sales_order_model->addSale($data, $products, $payment)) {
            ...
            if ($quote_id || $sale_id) {
                if ($quote_id) {
                    ...
                    $dataQuote = $this->sales_order_model->getQuoteByID($quote_id);
                    $items               = $this->sales_order_model->getAllQuoteItems($quote_id);
                    $this->data['dataCustomer'] =$this->companies_model->getCompanyByID($dataQuote->customer_id);
                    // var_dump($this->companies_model->getCompanyByID($dataQuote->customer_id))
                } elseif ($sale_id) {
    ...
    public function getSalesOrder($biller_id = null)
    {
        ...
        $authorization = "<a href='#' class='po approve_sale_order' title='<b>" . $this->lang->line("approve_sale_order") . "</b>' data-content=\"<p>"
            . lang('r_u_sure') . "</p><a class='btn btn-success po-delete' href='" .admin_url('sales_order/getAuthorization/$1') . "'>"
            . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-check\"></i> "
            . lang('approve_sale_order') . "</a>";


        $unapproved = "<a href='#' class='po unapprove_sale_order' title='<b>" . $this->lang->line("unapprove_sale_order") . "</b>' data-content=\"<p>"
            . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('sales_order/getunapproved/$1') . "'>"
    ...
    function getunapproved($id=NULL) {
        ...
        if($this->sales_order_model->getunapproved($id)){
            ...
            $this->bpas->send_json(['error' => 0, 'msg' => lang('sale_order_unapproved')]);
            redirect($_SERVER["HTTP_REFERER"]); 
        }else{
            $this->session->set_flashdata('error', validation_errors());
            die();
